  ...
  setKC_DC(ws: Worksheet | undefined) {
    ...
    if (ws) {
      let rows: string[] = this.claims.map((claim: any, i: number) => {
        ...
        let serialLotNo = claim.serialLotNo ? claim.serialLotNo : ''
        ...
        let useAppear = claim.useAppearance ? claim.useAppearance : ''
        ...
        let functionAp = `${claim.functionAppearance ? claim.functionAppearance : ''}`
        let importance = claim.importance ? claim.importance?.split(' ')[0] : ''
        ...
        let values = [
          uploadOBL, submitCustomer, analysisPIC, returnStyle, modelClassification, calendarYear, partReceivingDate, claimRegisterDate, receiveInformationDate, dueDate, finalReportUploadDate, finalReportSubmitDate, claimNo, customerNo, sideNo, salePIC, customerNameB, saleCompany, RGAS_no, ktcModelNo, ktcModelCode, qty, productLotNo, serialLotNo, productMonth, commercialDistribution, useAppear, occurredLocation, descriptionJP, descriptionENG, ktcAnalysisResult, functionAp, importance, ng, notAccept, noAbnormality, withinSpec, notRecurred, difference, causeByCustomer,
